...
public class NiFer extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        semes.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "First Sem", "Second Sem", "Summer" }));
    ...
    ArrayList<Integer> subunits = new ArrayList<>();
    ...
    for (int i = 0; i < subgrades.size(); i++) {
        ttlgrades += subgrades.get(i) * subunits.get(i);
        ttlunits += subunits.get(i);
    }
    ...
    for (int i = 0; i < subcodes.size(); i++) {
        for (int j = i + 1; j < subcodes.size(); j++) {
            if (subcodes.get(i).compareTo(subcodes.get(j)) > 0) {
                ...
                int tempUnit = subunits.get(i);
                subunits.set(i, subunits.get(j));
                subunits.set(j, tempUnit);
    ...
    for (int i = 0; i < subcodes.size(); i++) {
        // Format the subgrade to 2 decimal places
        String formattedGrade = String.format("%.2f", subgrades.get(i));
        Object[] rowData = {subcodes.get(i), subnames.get(i), subunits.get(i), formattedGrade};
    ...
    if (isEmpty) {
        ...
        gradeeee.setText(String.format("%.2f", ttlgrades/ttlunits));
    }
