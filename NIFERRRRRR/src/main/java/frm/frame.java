...
import Classeses.Semester;
import Classeses.Student;
...
public class frame extends javax.swing.JFrame {
    ...
    public frame() {
        ...
        MongoDatabase database4 = Semester.connectToDatabase();
        List<Semester> semesters = Semester.getAllSemestersFromDB(database4);
    ...
    private void initComponents() {
        ...
        GradePanel.add(stuList, new org.netbeans.lib.awtextra.AbsoluteConstraints(190, 10, 390, 30));
        ...
        GradePanel.add(jLabel15, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 90, -1, 28));
        ...
        GradePanel.add(jLabel17, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 10, -1, 28));

        jLabel18.setFont(new java.awt.Font("Sitka Display", 1, 18)); // NOI18N
        jLabel18.setText("SEMESTER");
        GradePanel.add(jLabel18, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 50, -1, 28));

        syear.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "2023-2024", "2024-2025" }));
        GradePanel.add(syear, new org.netbeans.lib.awtextra.AbsoluteConstraints(190, 90, 390, 30));

        sem.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "First Sem", "Second Sem" }));
        GradePanel.add(sem, new org.netbeans.lib.awtextra.AbsoluteConstraints(190, 50, 390, 30));
        ...
        GradePanel.add(jButton10, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 130, 130, 40));
        ...
        stuID2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                stuID2ActionPerformed(evt);
            }
        });
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addComponent(jTabbedPane5, javax.swing.GroupLayout.PREFERRED_SIZE, 600, javax.swing.GroupLayout.PREFERRED_SIZE)
        );
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(jTabbedPane5, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 600, javax.swing.GroupLayout.PREFERRED_SIZE)
        );
    ...
    private void stuID2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_stuID2ActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_stuID2ActionPerformed
    ...
    MongoDatabase database4 = Enrollment.connectToDatabase(); 
    List<Enrollment> enrollments = Enrollment.getAllEnrollmentFromDB(database4);

    // Connect to the Student database
    MongoDatabase database = Student.connectToDatabase();
    List<Student> students = Student.getAllStudentsFromDB(database);
    
    MongoDatabase database2 = Semester.connectToDatabase();
    List<Semester> seme = Semester.getAllSemestersFromDB(database2);

    // Get selected values from combo boxes
    String name = (String) stuList.getSelectedItem();
    String semester = (String) sem.getSelectedItem();
    String schoolYear = (String) syear.getSelectedItem();
    String id = null;
    double semID = 0;
    
    for (Semester semem : seme) {
        semID = semem.getSemesterID();
    }

    // Ensure valid selections
    if (name == null || semester == null || schoolYear == null) {
        JOptionPane.showMessageDialog(this, "Please select a student, semester, and school year.");
        return;
    }
    ...
    // Ensure the student ID was found
    if (id == null) {
        JOptionPane.showMessageDialog(this, "Student not found.");
        return;
    }

    // Lists to store subject codes and grades
    ArrayList<String> subjcd = new ArrayList<>();
    ArrayList<Double> subjgr = new ArrayList<>();

    // Collect subject codes and grades for the selected student, semester, and school year
    for (Enrollment roll : enrollments) {
        if (id.equals(roll.getStudentID()) && semID == roll.getSemesterID()) {
            // Add subject codes to the list
            for (String code : roll.getSubjectCode()) {
                subjcd.add(code);
            }

            // Add grades to the list, with type checking
            for (Object grade : roll.getGrade()) {
                if (grade instanceof Double) {
                    subjgr.add((Double) grade);
                } else if (grade instanceof Integer) {
                    subjgr.add(((Integer) grade).doubleValue()); // Convert Integer to Double
                } else {
                    System.err.println("Unexpected grade type: " + grade.getClass());
                }
            }
        }
    }

    // Update the table model with collected data
    if (subjcd.isEmpty() || subjgr.isEmpty()) {
        JOptionPane.showMessageDialog(this, "No subjects found for the selected student, semester, and school year.");
    } else {
        for (int i = 0; i < subjcd.size(); i++) {
            model.addRow(new Object[]{subjcd.get(i), subjgr.get(i)});
        }
    }
