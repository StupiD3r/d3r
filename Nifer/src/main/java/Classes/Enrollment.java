...
if (gradeValue == null) {
    Grade = 0.0; // Assign a default grade if it's null
    System.err.println("Grade is null for StudentID: " + StudentID + ", SubjectCode: " + SubjectCode);
} else if (gradeValue instanceof Double) {
    Grade = (Double) gradeValue;
} else if (gradeValue instanceof Integer) {
    Grade = ((Integer) gradeValue).doubleValue();
} else if (gradeValue instanceof String) {
    try {
        Grade = Double.parseDouble((String) gradeValue);
    } catch (NumberFormatException e) {
        Grade = 0.0; // Default value if parsing fails
        System.err.println("Invalid grade format for StudentID: " + StudentID + ", SubjectCode: " + SubjectCode);
    }
} else {
    Grade = 0.0; // Default for unexpected types
